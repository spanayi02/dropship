  ...
  return (
    <article
      className={cn(
        ...
        "shadow-[0_1px_2px_rgba(0,0,0,0.06)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_32px_-16px_rgba(0,0,0,0.35)]",
        className
      )}
    >
      {/* Badges */}
      <div className="absolute top-2.5 left-2.5 z-10 flex flex-col gap-1.5">
        {isSale && (
          <span className="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-bold bg-destructive text-white">
            <Zap className="h-2.5 w-2.5" />
            Save {discount}%
          </span>
        )}
        {isNew && !isSale && (
          <span className="inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-bold bg-[var(--emerald)] text-[var(--emerald-foreground)]">
            New
          </span>
        )}
      </div>
...
export function ProductCardSkeleton({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "flex flex-col rounded-2xl bg-card shadow-[0_1px_2px_rgba(0,0,0,0.06)] overflow-hidden animate-pulse",
        className
      )}
    >
